...
const PublicMap = ({
  ...
  onFiltreChange  = null,
  onCountChange   = null,   // ← callback pour remonter le nb de prestataires en ligne (landing)
}) => {
  const isUserMode = prestatairesProps !== null || onSelectSource !== null;
  ...
  return (
    <div style={s.wrapper} onClick={() => { setShowSrcPicker(false); setShowRayonCtrl(false); }}>
      ...
      <div style={s.filterBar}>
        ...
        {isMobile ? (
          ...
          <div style={s.selectWrap}>
            <select
              style={s.select}
              value={filtreMetier}
              onChange={(e) => { setFiltreMetier(e.target.value); setSelected(null); }}
            >
              {metiersDispos.map((m) => (
        ...
        {isUserMode && (
          <button
            style={{ ...s.styleBtn, background: showRayonCtrl ? "rgba(201,168,76,0.2)" : "rgba(255,255,255,0.04)", color: "#c9a84c", border: "1px solid rgba(201,168,76,0.25)", flexShrink: 0 }}
            onClick={(e) => { e.stopPropagation(); setShowRayonCtrl((x) => !x); setShowSrcPicker(false); }}>
            {rayonLocal} km
          </button>
        )}
        ...
        <div style={s.styleSelector}>
          {MAP_STYLES.map((ms) => (
            <button key={ms.id}
              ...
              {/* Mobile : icône seule pour économiser la place */}
              {isMobile ? ms.label.split(" ")[0] : ms.label}
            </button>
          ))}
        </div>
      </div>
      ...
      {isUserMode && showRayonCtrl && (
        <div style={s.rayonPanel} onClick={(e) => e.stopPropagation()}>
          ...
          <input type="range" min={1} max={100} value={rayonLocal}
            ...
            style={{ width: "100%", height: 4, borderRadius: 2, outline: "none", cursor: "pointer", WebkitAppearance: "none", appearance: "none", background: "linear-gradient(to right, #c9a84c " + rayonPct + "%, rgba(201,168,76,0.15) " + rayonPct + "%)" }}
          />
          <button style={s.rayonSaveBtn} disabled={rayonSaving}
            onClick={() => { onSaveRayon && onSaveRayon(); setShowRayonCtrl(false); }}>
            {rayonSaving ? "Enregistrement..." : "Sauvegarder ce rayon"}
          </button>
        </div>
      )}
      ...
      <div style={s.mapContainer}>
        ...
        >
          ...
          {isUserMode && savedLocations.map((loc) => (
            <Marker key={loc._id} longitude={loc.longitude} latitude={loc.latitude} anchor="center">
              <div
                style={{ fontSize: 20, cursor: "pointer", opacity: activeSource === "saved:" + loc._id ? 1 : 0.45, transform: activeSource === "saved:" + loc._id ? "scale(1.15)" : "scale(1)", transition: "all 0.2s", filter: "drop-shadow(0 2px 4px rgba(0,0,0,0.5))" }}
                onClick={(e) => { e.stopPropagation(); onSelectSource && onSelectSource("saved:" + loc._id); }}>
                📍
              </div>
            </Marker>
          ...
          {filtres.map((p) => {
            ...
            return (
              <Marker key={p._id} longitude={lng} latitude={lat} anchor="center"
                onClick={(e) => { e.originalEvent.stopPropagation(); setSelected(isActive ? null : p); }}>
                <div
                  style={{ ...s.marker, background: color, boxShadow: isActive ? "0 0 0 3px " + color + "88, 0 4px 20px " + color + "99" : "0 2px 14px " + color + "66", transform: isActive ? "scale(1.25)" : "scale(1)" }}
                  title={p.prenom + " " + p.nom + " — " + metierNom}>
                  <span style={{ fontSize: 15 }}>{icon}</span>
                  <div style={{ ...s.pulse, borderColor: color + "55" }} />
                </div>
...
const s = {
  wrapper:     { display: "flex", flexDirection: "column", width: "100%", height: "100%", background: "#0a0804" },
  filterBar:   { display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", background: "rgba(10,8,4,0.96)", borderBottom: "1px solid rgba(201,168,76,0.1)", flexWrap: "nowrap", flexShrink: 0, overflowX: "auto" },
  filterLeft:  { flexShrink: 0 },
  liveChip:    { display: "inline-flex", alignItems: "center", gap: 5, fontSize: 11, color: "rgba(245,240,232,0.45)", whiteSpace: "nowrap" },
  liveDot:     { width: 6, height: 6, borderRadius: "50%", background: "#4caf6e", display: "inline-block", animation: "pulse-dot 1.8s ease-in-out infinite" },
  ...
  select: {
    ...
    padding: "6px 8px", cursor: "pointer", minWidth: 0,
  },
  selectArrow: {
    paddingRight: 8, fontSize: 16, color: "#c9a84c",
    pointerEvents: "none", transform: "rotate(90deg)", lineHeight: 1, flexShrink: 0,
  },
