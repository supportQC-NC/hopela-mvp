...
const LoginScreen = () => {
  ...
  const handleRegister = async (e) => {
    ...
    if (role === "prestataire") {
      if (!telephone.trim()) { setError("Le numéro de téléphone est obligatoire."); setLoading(false); return; }
      if (!ridet.trim())     { setError("Le RIDET est obligatoire."); setLoading(false); return; }
    }
  ...
  return (
    <div className="login-root">
      ...
      <div className="login-panel">
        ...
        {/* Tabs */}
        <div className="login-tabs">
          <button className={"login-tab" + (tab === "login" ? " active" : "")} onClick={() => { setTab("login"); setError(""); setPendingValidation(false); }}>Connexion</button>
          <button className={"login-tab" + (tab === "register" ? " active" : "")} onClick={() => { setTab("register"); setError(""); setPendingValidation(false); }}>Créer un compte</button>
        </div>
          ...
          <>
            ...
            <form onSubmit={handleLogin}>
              <div className="login-field">
                ...
                <div className="login-input-wrap">
                  <span className="login-input-icon">✉️</span>
                  <input type="email" className="login-input" placeholder="vous@example.com" value={email} onChange={(e) => setEmail(e.target.value)} required autoComplete="email" />
                </div>
              </div>
              <div className="login-field">
                <label className="login-label">Mot de passe</label>
                <div className="login-input-wrap">
                  <span className="login-input-icon">🔑</span>
                  <input type="password" className={"login-input" + (error ? " error" : "")} placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} required autoComplete="current-password" />
                </div>
              </div>
              <div className="login-forgot"><a href="/forgot-password">Mot de passe oublié ?</a></div>
              <button type="submit" className="login-btn" disabled={loading}>
                {loading ? <span className="login-spinner" /> : null}
                {loading ? "Connexion..." : "Se connecter →"}
              </button>
          ...
          <>
            ...
            {pendingValidation ? (
              <div style={{ textAlign: "center", padding: "20px 0" }}>
                ...
                <h1 className="login-form-title" style={{ fontSize: 24, marginBottom: 12 }}>Compte créé !</h1>
                <div style={{ background: "rgba(201,168,76,0.08)", border: "1px solid rgba(201,168,76,0.25)", borderRadius: 12, padding: "20px", marginBottom: 20 }}>
                  <p style={{ fontSize: 14, color: "rgba(245,240,232,0.7)", lineHeight: 1.7 }}>
                    Votre compte prestataire est <strong style={{ color: "#c9a84c" }}>en attente de validation</strong> par un administrateur Hopela.<br /><br />
                    Vous recevrez un email dès que votre compte sera activé. Cela prend généralement moins de 24h.
                  </p>
                </div>
                <button className="login-btn" onClick={() => { setTab("login"); setPendingValidation(false); }}>
                ...
                <form onSubmit={handleRegister}>
                  <div className="login-row">
                    <div className="login-field">
                      ...
                      <div className="login-input-wrap">
                        <span className="login-input-icon">👤</span>
                        <input type="text" className="login-input" placeholder="Jean" value={prenom} onChange={(e) => setPrenom(e.target.value)} required />
                      </div>
                    </div>
                    <div className="login-field">
                      <label className="login-label">Nom</label>
                      <div className="login-input-wrap">
                        <span className="login-input-icon">👤</span>
                        <input type="text" className="login-input" placeholder="Dupont" value={nom} onChange={(e) => setNom(e.target.value)} required />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="login-field">
                    ...
                    <div className="login-input-wrap">
                      <span className="login-input-icon">✉️</span>
                      <input type="email" className="login-input" placeholder="vous@example.com" value={regEmail} onChange={(e) => setRegEmail(e.target.value)} required autoComplete="email" />
                    </div>
                  </div>
                  ...
                  <div className="login-field">
                    ...
                    <div className="login-input-wrap">
                      <span className="login-input-icon">🔑</span>
                      <input type="password" className="login-input" placeholder="Min. 6 caractères" value={regPass} onChange={(e) => setRegPass(e.target.value)} required minLength={6} autoComplete="new-password" />
                    </div>
                  </div>
                  ...
                  <div className="login-field">
                    ...
                    <div className="login-input-wrap">
                      <span className="login-input-icon">🛡️</span>
                      <select className="login-select" value={role} onChange={(e) => { setRole(e.target.value); setTelephone(""); setRidet(""); }}>
                        <option value="user">Particulier — je cherche un prestataire</option>
                        <option value="prestataire">Prestataire — je propose mes services</option>
                      </select>
                    </div>
                  ...
                  {role === "prestataire" && (
                    <>
                      <div className="login-field">
                        <label className="login-label">Téléphone professionnel *</label>
                        <div className="login-input-wrap">
                          <span className="login-input-icon">📞</span>
                          <input type="tel" className="login-input" placeholder="+687 XX XX XX" value={telephone} onChange={(e) => setTelephone(e.target.value)} required />
                        </div>
                      </div>
                      <div className="login-field">
                        ...
                        <div className="login-input-wrap">
                          <span className="login-input-icon">🏢</span>
                          <input type="text" className="login-input" placeholder="Ex : 1234567-001" value={ridet} onChange={(e) => setRidet(e.target.value)} required />
                        </div>
                        <div style={{ fontSize: 11, color: "rgba(245,240,232,0.3)", marginTop: 5 }}>
                          Registre de l'Industrie, du Commerce et des Métiers (Nouvelle-Calédonie)
                        </div>
                      </div>
                      <div style={{ background: "rgba(201,168,76,0.06)", border: "1px solid rgba(201,168,76,0.15)", borderRadius: 10, padding: "12px 14px", marginBottom: 16, fontSize: 12, color: "rgba(245,240,232,0.5)", lineHeight: 1.6 }}>
                        ℹ️ Votre compte sera activé après validation par un administrateur. Vous recevrez un email de confirmation.
                      </div>
                    </>
                  )}
