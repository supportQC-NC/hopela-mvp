...
const useSocket = () => {
  ...
  useEffect(() => {
    ...
    // ── Listeners NOMMÉS — obligatoire pour .off() ciblé
    // Sans ça, .off("event") retire TOUS les listeners, y compris ceux
    // d'autres instances du hook, cassant le socket pour les remontages suivants
    const onLocationUpdated = ({ userId, longitude, latitude }) => {
      const store      = window.__hopela_redux_store__;
      const prestataires = store?.getState?.()?.location?.prestataires || [];
      const existe     = prestataires.some((p) => p._id === userId);

      if (existe) {
        dispatchRef.current(updatePrestairePosition({ userId, longitude, latitude }));
      } else {
        // Prestataire absent (était offline) → fetch ses infos puis l'ajouter
        fetch(`${SOCKET_URL}/api/users/prestataires/positions/public`)
          .then((r) => r.ok ? r.json() : [])
          .then((list) => {
            const prestaireData = list.find((p) => p._id === userId) || null;
            dispatchRef.current(updatePrestairePosition({
              userId, longitude, latitude, prestaireData,
            }));
          })
          .catch(() => {
            dispatchRef.current(updatePrestairePosition({ userId, longitude, latitude }));
    ...
    const onTrackingStopped = ({ userId }) => {
      // Retire le prestataire du tableau Redux → disparaît de la carte immédiatement
      dispatchRef.current(removePrestataire({ userId }));
    };

    // Enregistrer avec référence nommée
    socket.on("location_updated", onLocationUpdated);
    socket.on("tracking_stopped", onTrackingStopped);
